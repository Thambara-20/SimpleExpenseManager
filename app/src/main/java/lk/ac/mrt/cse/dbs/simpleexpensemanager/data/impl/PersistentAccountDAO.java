...
public class PersistentAccountDAO implements AccountDAO {
    ...
    public Account getAccount(String accNo) throws InvalidAccountException {
        Account account;
        database = manager.getReadableDatabase();
        String current_state = accountNo + " = ?";
        String[] conditionargs = {accNo};
        String[] column_names = {accountNo, bankName, accountHolderName, balance};

        Cursor cursor = database.query(accountTable, column_names, current_state, conditionargs, null, null, null);
        if (cursor == null) {
            throw new InvalidAccountException("Acc NO : " +accNo+ " is Invalid!");
        } else {
            cursor.moveToFirst();
            account = new Account(accNo, cursor.getString(cursor.getColumnIndex(bankName)), cursor.getString(cursor.getColumnIndex(accountHolderName)), cursor.getDouble(cursor.getColumnIndex(balance)));
        }
        cursor.close();
        return account;
    }
