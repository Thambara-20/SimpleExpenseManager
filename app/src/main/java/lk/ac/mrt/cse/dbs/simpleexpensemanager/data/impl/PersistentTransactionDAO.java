...
public class PersistentTransactionDAO implements TransactionDAO {
    ...
    @Override
    public void logTransaction(Date dateT, String accNo, ExpenseType type, double amount) {
        ...
        contentValues.put(SQLiteManager.amount, amount);
    ...
    public List<Transaction> getPaginatedTransactionLogs(int lim) throws ParseException {
        List<Transaction> Transactios = new ArrayList<>();
        database = manager.getReadableDatabase();
        String[] column_names = {date, accountNo, expenseType, amount};
        Cursor cursor = database.query(transactionTable, column_names, null, null, null, null, null);
        ...
        while (cursor.moveToNext()) {
            ...
            Transaction transaction = new Transaction(tempDate1, accNo, eType, amnt);
            Transactios.add(transaction);
        }
        cursor.close();
        if (Count <= lim) return Transactios;

        return Transactios.subList(Count - lim, Count);
    }
}
